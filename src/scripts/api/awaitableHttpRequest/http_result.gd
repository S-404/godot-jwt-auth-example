...
var cookies: Dictionary			## The response cookies.
...
static func _from_array(a: Array) -> HTTPResult:
	...
	h.cookies = _cookies_to_dict(a[2] as PackedStringArray)
...
static func _cookies_to_dict(headers_arr: PackedStringArray) -> Dictionary:
	var dict := {}
	for h in headers_arr:
		var h_split := h.split(":")
		if(h_split[0] == "Set-Cookie"):
			var cookie := h_split[1].strip_edges()
			var cookie_name := h_split[1].split("=")[0].strip_edges()
			dict[cookie_name] = cookie
